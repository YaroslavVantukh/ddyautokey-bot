...
async def welcome(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1. Запускаємо таймер на видалення системного повідомлення ("Ключ запросив...") через 1 годину (3600 сек)
    ...
    thread_id = msg.message_thread_id
    chat_name = chat.title or "нашій групі"
    
    # 2. Формуємо текст привітання локсміта
    text = (
        f"👋 Привіт, <b>{', '.join(names)}</b>! Вітаємо в групі <b>{chat_name}</b>! 🔑\n\n"
        f"Розкажіть, який у вас рівень навичок локсміта і де ви знаходитесь?"
    )
    
    # 3. Надсилаємо привітання в ту саму гілку і теж ставимо на видалення через 1 годину
...
async def clean_system_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    chat = update.effective_chat
    if msg and chat:
        # Видаляємо всі інші системні повідомлення (покинув групу тощо) через 1 хвилину
        asyncio.create_task(delete_later(context.bot, chat.id, msg.message_id, delay=60))
...
def main():
    ...
    # 1. Обробляємо нових користувачів (привітання + видалення системного повідомлення через годину)
    application.add_handler(MessageHandler(filters.StatusUpdate.NEW_CHAT_MEMBERS, welcome))
    
    # 2. Обробляємо ВСІ ІНШІ системні повідомлення ("покинув групу", "змінив назву", "закріпив" тощо)
    ...
    # 3. Потім обробляємо всі звичайні повідомлення, ігноруючи системні та команди
