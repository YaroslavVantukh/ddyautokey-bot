...
def get_rank_tag(rep: int, lang: str) -> str:
    tags = TAG_RANKS.get(lang, TAG_RANKS["en"])
    current_tag = ""
    ...
    try:
        ...
        if new_positive and not old_positive:
            ...
            if cur.rowcount:
                ...
                conn.commit()
                receiver_row = conn.execute(
                    "SELECT reputation, language_code FROM users WHERE chat_id=? AND user_id=?",
                    (chat_id, receiver_id),
                ).fetchone()
                if receiver_row:
